...
#ifndef NODE_BLEND_SRC_PNG_IO_HPP
#define NODE_BLEND_SRC_PNG_IO_HPP
...
void reduce_8  (T const& in, image_data_8 & out, octree<rgb> trees[], unsigned limits[], unsigned levels, std::vector<unsigned> & alpha)
{
    ...
    for (unsigned y = 0; y < height; ++y)
    {
        ...
        {
            unsigned val = row[x];
            rgb c(U2RED(val), U2GREEN(val), U2BLUE(val));
            byte index = 0;
            int idx = -1;
        ...
        for (unsigned x = 0; x < width; ++x)
        {
            ...
            for(unsigned j=TRANSPARENCY_LEVELS-1; j>0; j--){
                if (cols[j]>0 && U2ALPHA(val)>=limits[j]) {
                    trees[j].insert(rgb(U2RED(val), U2GREEN(val), U2BLUE(val)));
                    break;
                }
            }
        }
                  ...
                  int compression = Z_DEFAULT_COMPRESSION, int strategy = Z_DEFAULT_STRATEGY)
{
    ...
    if (palette.size() > 16 )
    {
        ...
        for (unsigned y = 0; y < height; ++y)
        {
            ...
            image_data_8::pixel_type  * row_out = reduced_image.getRow(y);
